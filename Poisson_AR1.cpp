...
Poisson regression (AR(1))

 note that r == d in this case; length of y and v are equal
*/
...
double g(arma::vec y, double alpha, arma::vec v){
  ...
  vec tmp(d);
  for (int j = 0; j < d; j++){
    tmp(j) = exp(alpha + v(j)) - ( alpha + v(j) ) * y(j) + log(Factorial(y(j)));
  }
  return sum(tmp);
}//end of function g
...
//Quasi-Newton method
// [[Rcpp::export]]
List PoisAGHMLE(arma::mat Y, double v0_num, arma::vec x0, 
