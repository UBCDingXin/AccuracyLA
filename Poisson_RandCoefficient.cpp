...
double g(arma::vec y, arma::mat X, arma::vec beta, arma::vec v){
  ...
  for (int j = 0; j < d; j++){
    ...
    tmp(j) = exp( tmpeta ) - y(j) * tmpeta + log(Factorial(y(j)));
  }//
  return sum(tmp);
}//end of g()
...
arma::mat gdev2(arma::vec y, arma::mat X, arma::vec beta, arma::vec v) {
  ...
  for (int j = 0; j < d; j++){
    ...
    etadev1.subvec(1, c) = vectorise(X.row(j), 0);
    output = output + exp(tmpeta) * etadev1 * etadev1.t();
  }//
...
//Quasi-Newton method
// [[Rcpp::export]]
List PoisAGHMLE(arma::mat Y, arma::cube XCube, double v0_num, arma::vec x0, 
